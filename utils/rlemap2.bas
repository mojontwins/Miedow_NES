...
Dim As Integer x, y, xx, yy, nPant, mapW, mapH, i, j, dat, f, mapsize, decosize, scrsize, hasDecos, savings, msa, msb
...
For nPant = 0 To mapPants - 1
	...
	Else
		...
		Print #f, " };"
	End If
	If sclpGetValue ("scrsizes") <> "" Then Print #f, "// Screen size = " & scrsize & " bytes." & " Savings: " & (96 - scrsize)
	savings = savings + (96 - scrsize)
	msa = msa + scrsize
	msb = msb + 96
	If sclpGetValue ("scrsizes") <> "" Then Print #f, "// Should be = " & msb & ", is " & msa
Next nPant
Print #f, ""
If Command (7) = "scrsizes" Then Print #f, "// Total savings = " & savings
...
Print #f, "// Savings = " & (savings - 2 * mapW * mapH)
Print #f, ""
