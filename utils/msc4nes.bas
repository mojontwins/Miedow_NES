...
Dim As Integer f, f2, f3, i, j, k, clin, nPant, maxidx, scriptCount, binPt, offsPt
Dim As String linea, clausulas, o, inFileName, outFileName
Dim As Integer keepGoing, sDone, nSections, killing
Dim As String mandatory (2) = { "in", "out", "maxpants" }
...
If Not sclpCheck (mandatory ()) Or Val (sclpGetValue ("maxpants")) = 0 Then
	Print "MojonTwins Scripts Compiler 4 [NES] 20170330"
	Print "MT Engine NES MK2 v0.8+"
	Print
	Print "Usage:"
	Print "$ msc4nes in=script.spt out=mscnes.h maxpants=n [mapadjust=n]"
	Print
	System
End If

inFileName = sclpGetValue ("in")
outFileName = sclpGetValue ("out")
...
Print #f2, "// generated by msc4nes by the mojon twins"
Print #f2, ""
'Print #f2, "unsigned char sc_x, sc_y, sc_n, sc_c;"
	...
	Select Case lP (0)
		...
		Case "END_OF_LEVEL":
			...
			If sDone Then
				...
				' first write the scripts themselves as separate arrays
				For i = 0 To maxidx + 5
					If Len (script (i)) = 0 Then
						addresses (i) = 0 
					Else 	
					 	addresses (i) = i
					 	Print #f2, "const unsigned char ";
					 	printGenName f2, i, maxidx
					 	Print #f2, " [] = {"

					 	k = 0
					 	For j = 1 To Len (script (i))
					 		if k = 0 Then Print #f2, "	";
					 		Print #f2, "0x" & Lcase (Hex (Asc (Mid (script (i), j, 1)), 2));
					 		If j < Len (script (i)) Then Print #f2, ", ";
					 		k = k + 1: If k = 16 Then k = 0: Print #f2, ""
					 	Next j
					 	If k Then Print #f2, ""
					 	Print #f2, "};"
					 	Print #f2, ""
				...
				Print #f2, "const unsigned char * const script_pool_" & thisLevelConstantName & " [] = {"
				k = 0
				For j = 0 To maxidx + 5
					If k = 0 Then Print #f2, "	";
					If addresses (j) = 0 Then 
						Print #f2, "0"; 
					Else
						printGenName f2, j, maxidx
					End If
					If j < maxidx + 5 Then Print #f2, ", ";
					k = k + 1: If k = 8 Then k = 0: Print #f2, ""
				Next j
				If k Then Print #f2, ""
				Print #f2, "};"
				Print #f2, ""
...
Print #f2, "const unsigned char * const * const level_scripts [] = {"
k = 0
For j = 0 To levelNamesListIndex - 1
	If k = 0 Then Print #f2, "	";
	Print #f2, "script_pool_" & levelNamesList (j);
	If j < levelNamesListIndex - 1 Then Print #f2, ", ";
	k = k + 1: If k = 8 Then k = 0: Print #f2, ""
Next j
If k Then Print #f2, ""
Print #f2, "};"
...
Print #f2, "	script = script_pool [whichs];"
Print #f2, "    if (script == 0) return;"
Print #f2, ""
...
If actionsUsed (&H16) Then
	print #f2, "					case 0x16:"
	print #f2, "						// MOD FLAG sc_x, sc_y"
	print #f2, "						// Opcode: 16 sc_x sc_y"
	print #f2, "						readvxy ();"
	print #f2, "						flags [sc_x] %= sc_y;"
	print #f2, "						break;"
End If
...
If actionsUsed (&H17) Then
	Print #f2, "					case 0x17:"
	Print #f2, "						// DECRANGE sc_n, sc_x, sc_y"
	Print #f2, "						sc_n = read_vbyte ();"
	Print #f2, "						readvxy ();"
	Print #f2, "						flags [sc_n] = flags [sc_n] > sc_x ? flags [sc_n] - 1 : sc_y;"
	Print #f2, "						break;"
End If

If actionsUsed (&H18) Then
	Print #f2, "					case 0x18:"
	Print #f2, "						// INCRANGE sc_n, sc_x, sc_y"
	Print #f2, "						sc_n = read_vbyte ();"
	Print #f2, "						readvxy ();"
	Print #f2, "						flags [sc_n] = flags [sc_n] < sc_y ? flags [sc_n] + 1 : sc_x;"
	Print #f2, "						break;"
End If
...
If actionsUsed (&H20) Then
	...
	Print #f2, "						readvxy ();"
	Print #f2, "						ul = update_list;"
	Print #f2, "						set_map_tile (sc_x, sc_y, read_vbyte ());"
	Print #f2, "						close_update_cycle ();"
	Print #f2, "						break;"
End If
...
If actionsUsed (&HE3) Then
	Print #f2, "					case 0xE3:"
	Print #f2, "						ul = update_list;"
	Print #f2, "						p_s_ul (LINE_OF_TEXT_X, LINE_OF_TEXT_Y, script_texts [read_vbyte ()]);"
	Print #f2, "						close_update_cycle ();"
	Print #f2, "						break;"
End If
...
If actionsUsed (&HF4) Then
	...
	Print #f2, "							ul = update_list;"
	Print #f2, "							set_map_tile (sc_x >> 4, sc_x & 15, read_byte ());"
	Print #f2, "							close_update_cycle ();"
	Print #f2, "						}"
	Print #f2, "						break;"
End If
